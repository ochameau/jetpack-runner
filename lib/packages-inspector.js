...
function normalizeAndValidate(packagePath, json) {
  ...
  if ("main" in json) {
    // Normalize to module name, not filename and only relative path
    json.main = json.main.replace(/\.js$/, "");
    if (json.main.indexOf("./") !== 0)
      json.main = "./" + json.main;
    
    // Validate existing `main` attribute
    // we need to convert path on windows:
    let ospath = path.join.apply(null, json.main.replace(/^\.\//, "").split("/"));
    let mainFile = path.join(packagePath, ospath) + ".js";
    ...
    for each (let folder in json.lib) {
      if (folder.indexOf(json.main) <=2) { // folder is "." (=0) or "lib" (=2)
        foundIt = true;
        break;
      }
    }
    ...
    for each (let folder in json.lib) {
      let ospath = path.join.apply(null, folder.split("/"));
      let mainFile = path.join(packagePath, ospath, DEFAULT_PROGRAM_MODULE) + ".js";
      if (path.existsSync(mainFile) &&
          fs.statSync(mainFile).isFile()) {
