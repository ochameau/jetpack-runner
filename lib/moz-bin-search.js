...
exports.findBinaries = function () {
  ...
  if (process.env.ProgramFiles) {
    ...
    for(let x=0; x<searchDirs.length; x++) {
      ...
      for(let i=0; i<subDirs.length; i++) {
        ...
        for(let j=0; j<possibleNames.length; j++) {
          let exe = path.join(subDir,possibleNames[j]+".exe")
    ...
    let possibleNames = ["firefox","mozilla-firefox","iceweasel","thunderbird","xulrunner"];
  ...
  if (path.existsSync(binary) && binaries.indexOf(binary)==-1)
