...
exports.runRemote = function runRemote(options) {
  ...
  let args = ["-no-remote", "-profile", profile];
    ...
    quit: function () {
      ...
      function cleanProfile() {
        require("rm-rec").rm(profile, function(err) {
          ...
          require("rm-rec").rm(temporaryXPIFolder, options.quit);
        });
      }
      cleanProfile();
    }
...
exports.runWithin = function runWithin(options) {
  ...
  let temporaryXPI = require("temp").path("zipped-xpi.xpi");
  ...
  function uninstall() {
    uninstallCount++;
    if (!install || !install.addon || !install.addon.id) {
      if (uninstallCount < 120) // check again during 30s
        return require("timer").setTimeout(uninstall, 250);
      return;
    }
